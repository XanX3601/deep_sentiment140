...
def load_dataset(mode):
    ...
    if mode == 'test':
        ...
        dataframe = pd.concat(
            [pd.DataFrame(x_test), pd.DataFrame(y_test)], axis=1)
...
if __name__ == "__main__":
    ...
    (testdata1, testdata2) = dataset.split(
        split_ratio=[0.9, 0.1])  # test a larger and smaller set
    ...
    test_loss, test_acc = evaluate(model, test_iterator2, criterion)
